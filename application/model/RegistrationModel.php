...
class RegistrationModel
{
    ...
    public static function registerNewUser()
    {
        ...
        // send verification email : WARNING turning true : self::sendVerificationEmail($user_id, $user_email, $user_activation_hash)
        if (true) {
            Session::add('feedback_positive', Text::get('FEEDBACK_ACCOUNT_SUCCESSFULLY_CREATED'));
            self::verifyNewUser($user_id,"");
            return true;
        }
    ...
    public static function registerNewUserInside()
    {
        // clean the input
        $user_name = strip_tags(Request::post('user_name'));
        $user_email = strip_tags(Request::post('user_email'));
        ...
        // write user data to database
        if (!self::writeNewUserToDatabaseInside($user_name, $user_password_hash, $user_email,$user_role, time(), $user_activation_hash)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_ACCOUNT_CREATION_FAILED'));
            return false; // no reason not to return false here
        }
    ...
    public static function writeNewUserToDatabaseInside($user_name, $user_password_hash, $user_email,$user_role, $user_creation_timestamp, $user_activation_hash)
    {
        $database = DatabaseFactory::getFactory()->getConnection();
        // write new users data into database
        $sql = "INSERT INTO users (user_name, user_password_hash, user_email, user_account_type ,user_creation_timestamp, user_activation_hash, user_provider_type)
                    VALUES (:user_name, :user_password_hash, :user_email, :user_account_type,:user_creation_timestamp, :user_activation_hash, :user_provider_type)";
        $query = $database->prepare($sql);
        $query->execute(array(':user_name' => $user_name,
                              ':user_password_hash' => $user_password_hash,
                              ':user_email' => $user_email,
    ...
    public static function verifyNewUser($user_id, $user_activation_verification_code)
    {
        ...
        if ($query->rowCount() == 1) {
            ...
            self::addUserToFirebase($user_id);
            return true;
        }
    ...
    public static function addUserToFirebase($user){
      ...
      $reference = NULL;
      if ($type == 7) {
        $reference = $database->getReference('teachers/"'.$key."\"");
      }else{
        $reference = $database->getReference('students/"'.$key."\"");
      }
      $reference->set($entry);
    }
}
